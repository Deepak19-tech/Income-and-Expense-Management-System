document.addEventListener('DOMContentLoaded', () => {
  ...
  if (trendCanvas && window.Chart) {
    ...
    ] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { usePointStyle: true, boxWidth: 8, font: { family: 'DM Sans', weight: '600' } } } }, scales: { x: { grid: { display: false }, ticks: { color: '#758198' } }, y: { beginAtZero: true, grid: { color: '#edf1f6' }, ticks: { color: '#758198' } } } } });
  }
  const pieCanvas = document.getElementById('pieChart');
  if (pieCanvas && window.Chart) {
    new Chart(pieCanvas, { type: 'doughnut', data: { labels: readChartData('chart-category-labels'), datasets: [{ data: readChartData('chart-category-values'), backgroundColor: ['#1455c9','#35a7e9','#16865b','#ffbd38','#d34d63','#8c6de9'], borderWidth: 0, hoverOffset: 5 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '68%', plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, boxWidth: 8, padding: 16, font: { family: 'DM Sans', weight: '600' } } } } } });
  }
});
